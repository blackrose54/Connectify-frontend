...
const Chat: FC<ChatProps> = ({}) => {
  const { Friends, current, setCurrent, chats, Unread, setUnread,online,Recents } =
    useSocket();
    ...
    <div className="containerCustom grow">
      ...
      {sortedEntries.map((entry, key) => {
        ...
        return (
          ...
          >
            <div className={`${online && online.has(friend.id) && online.get(friend.id) == true?"image-container-online":"image-container-offline"}  h-10 w-10`}>
            <Image
              src={friend.image!}
              alt="avatar"              
              fill
              className={`rounded-full after:bg-white ring-2 ${online && online.has(friend.id) && online.get(friend.id) == true?"ring-green-400":"ring-slate-300"}`}
            />
            </div>
            <div>
              <p>{friend.name}</p>
